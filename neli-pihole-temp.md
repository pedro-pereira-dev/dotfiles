...
echo
podman --remote run -d --userns auto --restart always \
  --name neli-pihole-unbound \
  -p 5353:53/udp \
  -v /etc/unbound/unbound.conf:/etc/unbound/unbound.conf \
  docker.io/alpinelinux/unbound:latest

# enable unprivileged port
echo 'net.ipv4.ip_unprivileged_port_start = 0' > /etc/sysctl.d/99-unprivileged-port.conf
sysctl --system
