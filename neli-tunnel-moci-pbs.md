...
```
root@neli-tunnel-moci-pbs:~# ufw status verbose
Status: active
Logging: on (low)
...
echo 'Address = 10.1.10.23/32' >> /etc/wireguard/wg0.conf
...
podman run -d --restart always \
  --name neli-tunnel-moci-pbs-ssh \
  --network host \
  --health-cmd='["nc", "-z", "10.0.10.23", "22"]' \
  --health-on-failure restart \
  docker.io/alpine/socat:latest \
    tcp-listen:22,fork,reuseaddr,tcp-keepidle=10,tcp-keepintvl=5,tcp-keepcnt=3 \
    tcp:10.0.10.23:22,tcp-keepidle=10,tcp-keepintvl=5,tcp-keepcnt=3
podman run -d --restart always \
  --name neli-tunnel-moci-pbs-hawser-remote \
  --network host \
  --health-cmd='["nc", "-z", "10.0.10.23", "2376"]' \
  --health-on-failure restart \
  docker.io/alpine/socat:latest \
    tcp-listen:2376,fork,reuseaddr,tcp-keepidle=10,tcp-keepintvl=5,tcp-keepcnt=3 \
    tcp:10.0.10.23:2376,tcp-keepidle=10,tcp-keepintvl=5,tcp-keepcnt=3
podman run -d --restart always \
  --name neli-tunnel-moci-pbs \
  --network host \
  --health-cmd='["nc", "-z", "10.0.10.23", "8007"]' \
  --health-on-failure restart \
  docker.io/alpine/socat:latest \
    tcp-listen:8007,fork,reuseaddr,tcp-keepidle=10,tcp-keepintvl=5,tcp-keepcnt=3 \
    tcp:10.0.10.23:8007,tcp-keepidle=10,tcp-keepintvl=5,tcp-keepcnt=3

# setup hawser
mkdir -p /opt/podman/hawser
podman run -d --restart always \
  --name neli-tunnel-moci-pbs-hawser \
  --network host \
  -e PORT=2377 \
...
podman inspect --format='{{range .Config.Env}}{{println .}}{{end}}' neli-tunnel-moci-pbs-hawser | grep TOKEN | cut -d= -f2
