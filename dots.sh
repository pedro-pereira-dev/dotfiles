...
[ $# -ge 2 ] && [ "$1" = install ] && [ "$2" = gentoo ] &&
  curl -Lfs -- https://raw.githubusercontent.com/pedro-pereira-dev/dotfiles/refs/heads/main/utils/install-gentoo.sh | sh -s -- "$@"
...
dots_sync() {
  ...
  #     source_file "hosts/$_HOSTNAME/$_HOSTNAME.sh" ||
