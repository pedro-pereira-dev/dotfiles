...
# sets up prowlarr
mkdir -p /opt/podman/prowlarr
podman run -d --replace --restart always \
  --name nedi-prowlarr \
  --network host \
  -e TZ=Europe/Lisbon \
  -v /opt/podman/prowlarr:/config \
  --health-cmd='["curl", "-f", "http://127.0.0.1:9696"]' \
  --health-on-failure restart \
  lscr.io/linuxserver/prowlarr:latest

# sets up flaresolverr
podman run -d --replace --restart always \
  --name nedi-prowlarr-flaresolverr \
  --network host \
  -e TZ=Europe/Lisbon \
  --health-cmd='["curl", "-f", "http://127.0.0.1:8191"]' \
  --health-on-failure restart \
...
# Prowlarr
ufw allow in on eth0 from 10.0.0.0/8 to any port 9696 proto tcp
ufw allow in on eth0 from 172.16.0.0/12 to any port 9696 proto tcp
ufw allow in on eth0 from 192.168.0.0/16 to any port 9696 proto tcp
ufw enable
