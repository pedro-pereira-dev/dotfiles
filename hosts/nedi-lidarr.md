...
pct set 1011 -mp0 /mnt/pve/mnt-nas/nedi/media/,mp=/data
# enable protection
pct start 1011
pct enter 1011
...
# sets up lidarr
mkdir -p /opt/podman/lidarr
podman run -d --replace --restart always \
  --name nedi-lidarr \
  --network host \
  -e TZ=Europe/Lisbon \
  -v /data:/data \
  -v /opt/podman/lidarr:/config \
  --health-cmd='["curl", "-f", "http://127.0.0.1:8686"]' \
...
# Lidarr
ufw allow in on eth0 from 10.0.0.0/8 to any port 8686 proto tcp
ufw allow in on eth0 from 172.16.0.0/12 to any port 8686 proto tcp
ufw allow in on eth0 from 192.168.0.0/16 to any port 8686 proto tcp
ufw enable
