...
# sets up profilarr
mkdir -p /opt/podman/profilarr
podman run -d --replace --restart always \
  --name nedi-profilarr \
  --network host \
  -e TZ=Europe/Lisbon \
  -v /opt/podman/profilarr:/config \
  --health-cmd='["curl", "-f", "http://127.0.0.1:6868"]' \
...
# Profilarr
ufw allow in on eth0 from 10.0.0.0/8 to any port 6868 proto tcp
ufw allow in on eth0 from 172.16.0.0/12 to any port 6868 proto tcp
ufw allow in on eth0 from 192.168.0.0/16 to any port 6868 proto tcp
ufw enable
