...
-- custom commands
vim.keymap.set("n", "<leader><cr>", ":MyCommands<cr>", { silent = true })
...
require("lazy").setup({
	...
	spec = {
		...
		-- https://github.com/stevearc/conform.nvim
		{
			...
			cmd = { "Format", "FormatWithoutSaving", "SaveWithoutFormatting" },
			event = { "BufWritePre" },
			config = function()
				local format = vim.cmd.write
				local formatWithoutSaving = require("conform").format
				local saveWithoutFormatting = function()
					vim.b.disable_autoformat = true
					vim.cmd.write()
					vim.b.disable_autoformat = false
				end
				vim.api.nvim_create_user_command("Format", format, {})
				vim.api.nvim_create_user_command("FormatWithoutSaving", formatWithoutSaving, {})
				vim.api.nvim_create_user_command("SaveWithoutFormatting", saveWithoutFormatting, {})
		...
		{ "mason-org/mason.nvim", lazy = true, opts = { ui = { border = "single", width = 140 } } },
		...
		-- https://github.com/ibhagwan/fzf-lua
		{
			...
			cmd = { "FzfLua", "MyCommands" },
			config = function()
				vim.api.nvim_create_user_command("MyCommands", function()
					local function map_table(input_table, key_to_extract)
						local result = {}
						for _, item in ipairs(input_table) do
							if item[key_to_extract] then table.insert(result, item[key_to_extract]) end
						end
						return result
					end

					local commands = {
						{ action = "Format", description = "Neovim: format document" },
						{ action = "FormatWithoutSaving", description = "Neovim: format document without saving" },
						{ action = "Lazy sync", description = "Lazy: synchronize plugins" },
						{ action = "Lazy", description = "Lazy: open dialog" },
						{ action = "Mason", description = "Mason: open dialog" },
						{ action = "SaveWithoutFormatting", description = "Neovim: save document without formatting" },
					}

					require("fzf-lua").fzf_exec(map_table(commands, "description"), {
						actions = {
							["default"] = function(selected)
								for _, command in ipairs(commands) do
									if selected[1] == command.description then
