...
```
root@neli-tunnel-moci-wireguard-server:~# ufw status verbose
Status: active
Logging: on (low)
...
echo 'Address = 10.1.10.10/32' >> /etc/wireguard/wg0.conf
...
podman run -d --restart always \
  --name neli-tunnel-moci-wireguard-server-ssh \
  --network host \
